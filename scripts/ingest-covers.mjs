...
/** AniList descriptions run long, carry HTML line breaks, and often end mid
 *  sentence with promotional trailing lines ("Comes with an exclusive..."). This
 *  keeps the first couple of real sentences — enough to know what a book is
 *  about, short enough to sit under a cover without dominating the page. */
function trimSynopsis(raw) {
  if (!raw) return null;
  const text = raw
    .replace(/<br\s*\/?>/gi, " ").replace(/<[^>]+>/g, "")   // AniList sometimes returns light HTML
    .replace(/\(Source:.*?\)/gi, "").replace(/\s+/g, " ").trim();
  if (!text) return null;

  const sentences = text.match(/[^.!?]+[.!?]+/g) || [text];
  let out = "";
  for (const s of sentences) {
    if (out.length + s.length > 280 && out) break;   // stop once we have enough
    out += s;
    if (out.length > 160) break;                      // two sentences is usually plenty
  }
  return out.trim() || text.slice(0, 200).trim();
}
...
[
  ...
  for (const s of targets) {
    ...
    if (staleMeta(cache[`meta:${s.id}`])) {
      try {
        ...
        cache[`meta:${s.id}`] = {
          ...
          synopsis: trimSynopsis(media.description),
        };
    ...
    // A real synopsis beats no synopsis, and beats a hand-written one that's
    // gone stale. Only overwrite a hand-written blurb if none exists yet, so a
    // deliberately curated description in series.json is never clobbered.
    const synopsis = cache[`meta:${s.id}`]?.synopsis;
    ...
    for (let vol = 1; vol <= ceiling; vol++) {
      ...
      if (cached?.isbn13) { out.push({ seriesId: s.id, volume: vol, japanOnly: !!s.japanOnly, ...cached }); continue; }
      ...
      cache[key] = hit;
      out.push({ seriesId: s.id, volume: vol, japanOnly: !!s.japanOnly, ...hit });
      if (vol > s.volumes) {
